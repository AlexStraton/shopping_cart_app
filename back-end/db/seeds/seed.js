...
const seed = async ({ products, productsInCart, users }) => {
    ...
    const insertProductsString = format('INSERT INTO products (product_name, price, description, product_image_url) VALUES %L;', products.map(({ product_name, price, description, product_image_url }) => [product_name, price, description, product_image_url]));
    await db.query(insertProductsString);
    const insertProductsInCartString = format('INSERT INTO productsInCart (user_id, product_id, quantity) VALUES %L', productsInCart.map(({ user_id, product_id, quantity }) => [user_id, product_id, quantity]));
